...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    // (1) Create a disjoint sets data structure in which each cell of the maze 
    //     is represented as a separate item.
    DisjointSets dSet = new DisjointSets(horiz * vert); 
   
    // (2) Order the interior walls of the maze in a random order.
    int vWallsSize = (horiz - 1) * vert;
    int hWallsSize = horiz * (vert - 1);
    int[] walls = new int [vWallsSize + hWallsSize];
    for (int k = 0; k < walls.length; k++) {
    	walls[k] = k;
    }
    for (int k = walls.length - 1; k > 0; k--) {
    	int ranNum = randInt(k + 1); // ranNum = 0... k-1
    	int temp = walls[k];
    	walls[k] = walls[ranNum];
    	walls[ranNum] = temp;
    }
    ...
    for (int k = 0; k < walls.length; k++) {
    	...
    	int root1;
    	int root2;
    	if (walls[k] <= vWallsSize - 1) { 
    		x = walls[k] % (horiz - 1);
    		y = (walls[k]-1) / (horiz - 1);
    		// vertical wall (x, y) separates cell (x, y) from cell (x + 1, y)
    		root1 = dSet.find(y * horiz + x);
    		root2 = dSet.find(y * horiz + (x + 1));
    		if (root1 != root2) {
    			vWalls[x][y] = false;
    			dSet.union(root1, root2);
    		}
    	} else { // hWalls[i][j]
    		x = (walls[k] - vWallsSize) % horiz;
    		y = (walls[k] - vWallsSize) / horiz;
    		// horizontal wall (x, y) separates cell (x, y) from cell (x, y + 1)
    		root1 = dSet.find(y * horiz + x);
    		root2 = dSet.find((y + 1) * horiz + x);
    		if (root1 != root2){
    			hWalls[x][y] = false;
    			dSet.union(root1, root2);
    		}
