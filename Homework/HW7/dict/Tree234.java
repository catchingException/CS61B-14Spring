...
public class Tree234 extends IntDictionary {
  ...
  public void insert(int key) {
    ...
    leafInsert(previousNode, key);
  }
  
  public Tree234Node upstairs(Tree234Node node) {
    if (node == root) {
      ...
      updateChildRelationship(node, node.parent.child1, node.parent.child2);
      ...
      if (node == node.parent.child1) {
        ...
        updateChildRelationship(node, node.parent.child2, node.parent.child3);
        ...
        updateChildRelationship(node, node.parent.child3, node.parent.child4);
      }
      node.parent.keys++;
  ...
  public void updateChildRelationship(Tree234Node node, Tree234Node newNode1, Tree234Node newNode2) {
    if (node.child1 != null) { // if the node have childs
  ...
  public void leafInsert(Tree234Node previousNode, int key) {
    if (previousNode.keys == 1) {
      if (previousNode.key1 > key) {
        previousNode.key2 = previousNode.key1;
        previousNode.key1 = key;
      } else {
        previousNode.key2 = key;
      }
    } else if (previousNode.keys == 2) { 
      if (previousNode.key1 > key && previousNode.key2 > key) {
        previousNode.key3 = previousNode.key2;
        previousNode.key2 = previousNode.key1;
        previousNode.key1 = key;
      } else if (previousNode.key2 > key && previousNode.key1 < key) {
        previousNode.key3 = previousNode.key2;
        previousNode.key2 = key;
      } else {
        previousNode.key3 = key;
      }
    }
    previousNode.keys++;
    size++;
