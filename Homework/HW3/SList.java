...
public class SList {
  ...
  public void squish() {
    ...
    if (head!=null){
      SListNode cur = head;
      while(cur.next != null){
      //using .equals() instead of "==" to check whether the item of two nodes are the same
  ...
  public void twin() {
    ...
    //Part III
    SListNode cur = head; //manipulate the original linked list
    SListNode temp;//manipulate the new node in each iteration
    int ogSize = size;
    ...
    if (head != null){
      for (int i =0; i<ogSize ; i++){
        temp = cur;
        cur = cur.next;
        temp.next = new SListNode(temp.item);
        temp.next.next = cur;
        size++;
      }
    }
