...
public class SplayTree implements Dictionary {
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    while (node.parent != null) { // leave the while loop when the node become a root
      if (node.parent.parent != null) {
        if (node.parent.parent.leftChild != null) {
          if (node == node.parent.parent.leftChild.rightChild) {
            zigZag(node);
          } else {
            zigZig(node);
          }
        } else if (node.parent.parent.rightChild != null) {
          if (node == node.parent.parent.rightChild.leftChild) {
            zigZag(node);
          } else {
            zigZig(node);
          }
