...
public class AccountData implements Keyable {
   ...
   **/
  public void withdraw(int amt) throws BadTransactionException { // Part III
    if (amt < 0) {
      System.out.println("Error:  Tried to withdraw less than 0: " + amt);
      throw new BadTransactionException();
    } else if (amt <= balance) { // Part I: the subtle bug: It's doing deposit when we enter a negative number in the withdraw mode.
      balance = balance - amt;
    } else {
   ...
   **/
  public void deposit(int amt) throws BadTransactionException { // Part III
    if (amt >= 0) { // Part I: the subtle bug
      balance = balance + amt;
    } else {
