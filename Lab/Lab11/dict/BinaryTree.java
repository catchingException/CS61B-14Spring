...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    // Replace the following line with your solution.
    BinaryTreeNode node = findHelper((Comparable) key, root);
    BinaryTreeNode parNode = node.parent;
    if (node == null)
      return null;

    if (node.leftChild == null && node.rightChild == null) { // if removed node has no children
      if (parNode == null) {
        root = null;
      } else {
        if (parNode.leftChild == node)
          parNode.leftChild = null;
        else
          parNode.rightChild = null;
      }
    } else if (node.leftChild == null) { // if removed mode has one child (rightChild)
      if (parNode == null) {
        root = node.rightChild;
        node.rightChild.parent = null;
      } else {
        if (parNode.leftChild == node) {
          parNode.leftChild = node.rightChild;
          node.rightChild.parent = parNode;
        } else {
          parNode.rightChild = node.rightChild;
          node.leftChild.parent = parNode;
        }
      }
    } else if (node.rightChild == null) { // if removed mode has one child (leftChild)
      if (parNode == null) {
        root = node.leftChild;
        node.leftChild.parent = null;
      } else {
        if (parNode.leftChild == node) {
          parNode.leftChild = node.leftChild;
          node.leftChild.parent = parNode;
        } else {
          parNode.rightChild = node.leftChild;
          node.leftChild.parent = parNode;
        }
      }
    } else {
      BinaryTreeNode minNode = min(node.rightChild); // if removed mode has two children 
  ...
  /**
   *  min() find the minimum key in the tree (subtree).
   *  Repeatly go to left child until we reach a node with no left child.
   **/

  public BinaryTreeNode min(BinaryTreeNode subRoot) {
    BinaryTreeNode node = subRoot;
