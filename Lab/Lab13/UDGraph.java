...
public class UDGraph
{
  ...
  /**
   *  Returns true if v is a valid vertex number (0...n - 1); false otherwise.
   *  @param v the vertex.
  ...
  public UDGraph paths(int length) {
    // each "newGraph" only have the length of path is "length"
    // without any path it's length is smaller than "length"
    UDGraph newGraph = new UDGraph(vertices);
    if (length == 2) {
      newGraph = length2Paths();
    } else {
      UDGraph lastGragh = paths(length - 1);
      for (int i = 0; i < vertices; i++) {
        for (int j = 0; j < vertices; j++) {
          for (int k = 0; k < vertices; k++) { 
            if (lastGragh.hasEdge(i, k) && this.hasEdge(k, j)) { 
            // "this" is the original Graph which length of path = 1
            // "lastGragh" is the graph which length of path = ("length" - 1)
            // "newGraph" is the graph which length of path = "length"
              newGraph.addEdge(i, j); 
            } 
          }   
        }
