...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
    ...
    if (isValidNode()) {
      ...
      throw new InvalidNodeException(); // leave the method right away
    }
  }
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (c < 1)
        c = 1;
    if (isValidNode()) {
      if (firstChild == null) { // No child
        firstChild = new SibTreeNode(myTree, item);
        firstChild.parent = this;
        myTree.size++;
      } else if (c == 1) {
        SibTreeNode tempNode;
        tempNode = firstChild;
        firstChild = new SibTreeNode(myTree, item);
        firstChild.parent = this;
        myTree.size++;
        firstChild.nextSibling = tempNode;
      } else {
        SibTreeNode curNode;
        curNode = firstChild;
        int count = 2;
        while (curNode.isValidNode()) { // means it have the nextsibling
          if (count == c) {
            SibTreeNode tempNode;
            tempNode = curNode.nextSibling;
            curNode.nextSibling = new SibTreeNode(myTree, item);
            curNode.nextSibling.parent = this;
            myTree.size++;
            curNode.nextSibling.nextSibling = tempNode;
            return;
          } else if (! curNode.nextSibling().isValidNode()) {
            curNode.nextSibling = new SibTreeNode(myTree, item);
            curNode.nextSibling.parent = this;
            myTree.size++;
            return;
    ...
    if (isValidNode()) {
      if (firstChild == null) {
        if (parent == null) {
          ...
          while (! curNode.nextSibling.equals(this)) {
          curNode = (SibTreeNode)curNode.nextSibling();
          }
          curNode.nextSibling = nextSibling;
          valid = false;
          myTree.size--;
        }
      } 
